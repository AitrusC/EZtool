...
class CHtoolUI(QWidget):
    ...
    def _createPushBtn(self):
        ...
        self.select_left.setFixedHeight(35)
        self.select_right = QPushButton("InP")
        self.select_right.setFixedHeight(35)
        self.apply_btn = QPushButton(u"执行")
        self.apply_btn.setFixedHeight(40)
        self.select_left.setStyleSheet(qss)
        self.select_right.setStyleSheet(qss)
    ...
    def _createRadioB(self):
        ...
        qss = """QCheckBox {
                            ...
                            """
        self.add_selection_l = QCheckBox(u"添加选择")
        self.add_selection_r = QCheckBox(u"添加选择")
        self.add_selection_l.setStyleSheet(qss)
        self.add_selection_r.setStyleSheet(qss)
    ...
    def _createLayouts(self):
        ...
        self.main_layout.addLayout(self.hbox_layout1)
        self.hbox_layout1.addWidget(self.select_left, 4)
        self.hbox_layout1.addWidget(self.add_selection_l, 1)
        self.hbox_layout1.addWidget(self.select_right, 4)
        self.hbox_layout1.addWidget(self.add_selection_r, 1)
        self.hbox_layout2 = QHBoxLayout()
        self.main_layout.addLayout(self.hbox_layout2)
        self.hbox_layout2.addWidget(self.select_listl)
        self.hbox_layout2.addWidget(self.select_listr)
    ...
    def _addSelect(self, lorr):
        ...
        if lorr == "l":
            if not self.add_selection_l.checkState():
                self.select_listl.clear()
            for ite in range(self.select_listl.count()):
                text = self.select_listl.item(ite).text()
                if text in select_list:
                    select_list.remove(text)
            self.select_listl.addItems(select_list)
        else:
            if not self.add_selection_r.checkState():
                self.select_listr.clear()
            for ite in range(self.select_listr.count()):
                text = self.select_listr.item(ite).text()
                if text in select_list:
                    select_list.remove(text)
